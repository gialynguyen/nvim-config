...
local function require_modules()
  require("nvim")
  -- require("plugins") -- Uncomment if needed
  require("lazy-pkg-m")
  require("lsp")
  require("keymap")
end
...
local function setup_keymaps()
  ...
  -- ToggleTerm mappings (normal and insert mode)
  vim.api.nvim_set_keymap("t", "<C-t>", "<C-\\><C-n>:exe v:count1 . 'ToggleTerm'<CR>", { silent = true })
  vim.api.nvim_set_keymap("n", "<C-t>", ":exe v:count1 . 'ToggleTerm'<CR>", { silent = true })
  vim.api.nvim_set_keymap("i", "<C-t>", "<Esc><C-\\><C-n>:exe v:count1 . 'ToggleTerm'<CR>", { silent = true })

  -- ToggleTermToggleAll mappings (normal and insert mode)
  vim.api.nvim_set_keymap("t", "<C-\\>", "<C-\\><C-n>:ToggleTermToggleAll<CR>", { silent = true })
  vim.api.nvim_set_keymap("n", "<C-\\>", ":ToggleTermToggleAll<CR>", { silent = true })
  vim.api.nvim_set_keymap("i", "<C-\\>", "<Esc><C-\\><C-n>:ToggleTermToggleAll<CR>", { silent = true })
...
local function check_user_settings()
  if vim.fn.filereadable(vim.fn.glob("~/.config/nvim/user-settings.lua")) then
